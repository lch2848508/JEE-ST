    ...
    $.Zebra_Dialog = function() {
        ...
        plugin.settings = {}, "string" == typeof arguments[0] && (options.message = arguments[0]), ("object" == typeof arguments[0] || "object" == typeof arguments[1]) && (options = $.extend(options, "object" == typeof arguments[0] ? arguments[0] : arguments[1])), plugin.init = function() {
            ...
            if (plugin.message = $("<div>", {
                    ...
                    }), plugin.settings.max_height > 0 && (plugin.message.css("max-height", plugin.settings.max_height), plugin.isIE6 && plugin.message.attr("style", "height: expression(this.scrollHeight > " + plugin.settings.max_height + ' ? "' + plugin.settings.max_height + 'px" : "85px")')), plugin.settings.vcenter_short_message
                    ? $("<div>").html(plugin.settings.message).appendTo(plugin.message)
                    : plugin.message.html(plugin.settings.message), plugin.settings.source && "object" == typeof plugin.settings.source) {
                ...
                for (var f in plugin.settings.source)
                    switch (f) {
                        case "ajax" :
                            ...
                            } : plugin.settings.source[f], h = $("<div>").attr("class", "ZebraDialog_Preloader").appendTo(e);
            ...
            if (plugin.dialog.appendTo("body"), plugin.settings.show_close_button) {
                ...
                a && l.css({
                            ...
                            top : 15//(parseInt(a.css("height"), 10) + parseInt(a.css("paddingTop"), 10) + parseInt(a.css("paddingBottom"), 10) - l.height()) / 2
                        })
            }
        ...
        var draw = function() {
            var viewport_width = $(window).width(), viewport_height = $(window).height(), dialog_width = plugin.dialog.width(), dialog_height = plugin.dialog.height(), values = {
                left : 0,
                top : 0,
            ...
            if (plugin.dialog_left = void 0, plugin.dialog_top = void 0, $.isArray(plugin.settings.position) && 2 == plugin.settings.position.length && "string" == typeof plugin.settings.position[0] && plugin.settings.position[0].match(/^(left|right|center)[\s0-9\+\-]*$/)
                    && "string" == typeof plugin.settings.position[1] && plugin.settings.position[1].match(/^(top|bottom|middle)[\s0-9\+\-]*$/)
            ...
            if (plugin.settings.buttons !== !0 && !$.isArray(plugin.settings.buttons))
                return !1;
