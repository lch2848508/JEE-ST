...
public class PortalGroupServlet extends BaseServlet {

    private static final IDBHelper DBHELPER = RuntimeContext.getDbHelper();
    private static final long serialVersionUID = -1675429552446285962L;
    private final IPortalGroupService service = RuntimeContext.getPortalGroupService();
    ...
    protected void doRequest(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.equals("getInfo", operation))
            ...
            response.getWriter().println(listRecords());
     ...
     */
    private JSONObject listRecords() throws Exception {
        return RuntimeContext.getPortalGroupService().getGroupsAndItems();
    }
    ...
    private JSONObject saveRecord(final HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            record.setMemo(Convert.str2Bytes(getParamStr("memo")));
    ...
    private JSONObject getRecordInfo(final long id) throws Exception {
        JSONObject json = null;
        final PortalGroupRecord record = service.getRecord(null, id);
        if (record != null) {
            json = record.getJSON();
            json.put("r", true);
        } else {
