...
public class CodeAssistServlet extends BaseServlet {

    private static final IDBHelper DBHELPER = RuntimeContext.getDbHelper();
    private static final long serialVersionUID = -1675429552446285962L;
    private final ICodeAssistService service = RuntimeContext.getCodeAssistService();
    ...
    protected void doRequest(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        if (StringUtils.equals("getcodeassist", operation))
            response.getWriter().println(RuntimeContext.getCodeAssistService().getCodeAssist(getParamLong("version", Long.MIN_VALUE)));
    ...
    private JSONObject saveRecord(final HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            record.setCaption(getParamStr("caption"));
            record.setContent(getParamStr("content"));
            record.setPid(getParamLong("pid", -1l));
            record.setHelp(getParamStr("help"));
            record.setType(getParamInt("type"));
            record.setExtType(getParamStr("exttype"));
    ...
    private JSONObject getRecordInfo(final long id) throws Exception {
        JSONObject json = null;
        final CodeAssistRecord record = service.getRecord(null, id);
        if (record != null) {
            json = record.getJSON();
            json.put("r", true);
        } else {
