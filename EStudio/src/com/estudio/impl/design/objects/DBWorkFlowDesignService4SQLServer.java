...
public final class DBWorkFlowDesignService4SQLServer extends DBWorkFlowDesignService {
    ...
    protected String getInsertWFPropertySQL() {
        return "begin\n update sys_workflow_d_process set name = :name, descript = :descript, status = :status, version = :version,limit_num=:limit_num,limit_unit=:limit_unit where id = :id;\n if @@rowcount=0 begin\n insert into sys_workflow_d_process (id, name, descript, status, version, lastmodifydate,limit_num,limit_unit) values (:id, :name, :descript, :status, :version, getdate(),:limit_num,:limit_unit);\n end;\nend;";
    }
     ...
     * ¹¹Ôìº¯Êý
     */
    private DBWorkFlowDesignService4SQLServer() {
        super();
        initDBCommand();
    }

    private static IObjectWorkFlowService instance = new DBWorkFlowDesignService4SQLServer();
