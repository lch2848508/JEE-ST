...
public final class WFDBStorage4Oracle extends WFDBStorage {
    ...
    protected long generalProcessId(final Connection con) throws Exception {
        ...
        IDBCommand cmd = null;
        try {
            cmd = DBHELPER.getCommand(con, getSQL4GeneralProcessId());
            cmd.executeQuery();
            cmd.next();
    ...
    private JSONObject getCommonActivityUserTree(final Connection con, final WFActivity activity, final HashMap<String, Object> scriptContextParams) throws Exception, Exception {
        ...
        try {
            ...
            if (userIDS.size() != 0) {
                sql = "select * from (select distinct id, name, p_id from sys_department start with id in (select p_id from sys_userinfo where id in (" + StringUtils.join(userIDS.toArray(), ",") + ")) connect by id = prior p_id) t start with id = -1 connect by p_id = prior id order by sortorder";
                cmd.setSQL(sql);
                cmd.executeQuery();
                json = new JSONObject();
                json.put("activityName", activity.getName());
    ...
    protected String getSQL4CreateCase() {
        return "begin estudio_workflow.create_case(:process_id,:user_id,:process_type_id,:activity_name,:createdate,:process_durtion, :activity_duration);end;";
    }

    @Override
    protected String getSQL4AbabdonProcessByStep() {
        return "begin estudio_workflow.abandon_case_by_step(:step_id,:user_id); end;";
    }

    @Override
    protected String getSQL4RestoreProcessByStep() {
        return "begin estudio_workflow.restore_case_by_step(:step_id,:user_id); end;";
    }

    private WFDBStorage4Oracle() {
        super();

    }

    private static WFDBStorage4Oracle instance = new WFDBStorage4Oracle();
    ...
    protected String getSQL4SaveProcessVariableValue() {
        return "begin estudio_workflow.save_variable_value(:process_id,:var_name,:var_value); end;";
    }

    @Override
    protected String getSQL4SendToNextActitity() {
        return "begin estudio_workflow.send(:stepid,:activityname,:activitycaption,:userid,:limit_num,:limit_unit,:senddate,:activity_duration,:is_sendto_role); end;";
    }

    @Override
    protected String getSQL4FlagActivityFormSaved() {
        return "begin estudio_workflow.register_form_saved(:processid ,:activityname,:formid,:user_id); end;";
    }

    @Override
    protected String getSQL4BackProcess() {
        return "begin estudio_workflow.back(:step_id); end;";
    }

    @Override
    protected String getSQL4BackProcess2Creator() {
        return "begin estudio_workflow.back2creator(:step_id); end;";
    }

    @Override
    protected String getSQL4GeneralProcessId() {
        return "select estudio_workflow.get_processid() from dual";
    }

    @Override
    protected String getSQL4ProcessSteps() {
        return "select t.id,t.reciver_userid, to_char(t.reciver_finish_date,'YYYY-MM-DD Hh24:MI') finish_date, to_char(t.send_date,'YYYY-MM-DD Hh24:MI') send_date,t.activity_name,t.activity_caption,decode(t.step_type,1,'正常','退件') type,b.realname from sys_userinfo b, sys_workflow_r_step t where t.reciver_userid = b.id and t.process_id=:process_id order by t.send_date";
    }

    @Override
    protected String getSQL4ProcessIdeas() {
        return new StringBuilder().append("select a.id, a.idea_content, to_char(idea_date, 'YYYY-MM-DD Hh24:MI') idea_date, b.activity_caption, c.realname, replace(replace(estudio_attachment.getFileList('workflow_idea', a.id),'<a','<p><a'),'</a>','</a></p>') filelist\n") //
    ...
    protected String getSignCaseProcedureName() {
        return "estudio_workflow.sign_case";
    }

    @Override
    protected String getSQL4UpdateIdea() {
        return "update sys_workflow_r_ideas set idea_content=:idea_content,idea_date=sysdate where id=:step_id";
    }
