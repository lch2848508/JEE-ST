	...
	public class ColumnSymbol extends Symbol
	{
		...
		private function drawColumn(sprite:Sprite, attributes:Object, map:Map, point:MapPoint):void
		{
			...
			for (var j:uint=0; j < fields.length; j++)
			{
				...
				var val:Number=0;
				if(targetRegExp.test(attributes[fields[j].toString()])){
					val=attributes[fields[j].toString()].substring(0,attributes[fields[j].toString()].length-1);
				}else{
					val=attributes[fields[j].toString()];
				}
